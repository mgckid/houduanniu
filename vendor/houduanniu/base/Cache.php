...
class Cache
{
    ...
    public function erase($key)
    {
        ...
        if (true === is_array($cacheData)) {
            if (true === isset($cacheData[$key])) {
                ...
                throw new InternalServerErrorException("Error: erase() - Key '{$key}' not found.");
            }
        }
        return $this;
    }
    ...
    private function _checkCacheDir()
    {
        if (!is_dir($this->getCachePath()) && !mkdir($this->getCachePath(), 0775, true)) {
            throw new InternalServerErrorException('Unable to create cache directory ' . $this->getCachePath());
        } elseif (!is_readable($this->getCachePath()) || !is_writable($this->getCachePath())) {
            if (!chmod($this->getCachePath(), 0775)) {
                throw new InternalServerErrorException($this->getCachePath() . ' must be readable and writeable');
            }
        }
        return true;
    }
